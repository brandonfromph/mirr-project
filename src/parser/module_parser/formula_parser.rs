...
pub(super) fn parse_property(lines: &[&str], index: &mut usize) -> Result<PropertyDecl, MirrError> {
    if *index >= lines.len() {
        return Err(MirrError::parse_error(
            "[E147] Unexpected end of file in property declaration.",
        )
    ...
    let after_keyword = header.strip_prefix("property ").ok_or_else(|| {
        MirrError::parse_error("[E148] Malformed property declaration.")
    ...
    if name.is_empty() {
        return Err(MirrError::parse_error("[E149] Property name cannot be empty.")
            .with_span(Some(Span::full_line(*index as u32))));
    }
    ...
    *index += 1;
    ...
    if *index >= lines.len() {
        return Err(MirrError::parse_error(format!(
            "[E151] Property '{name}' not closed with '}}'."
        ))
        .with_span(Some(Span::full_line((*index).saturating_sub(1) as u32))));
    }
...
fn parse_always_or_implies_inner(inner: &str, name: &str) -> Result<PropertyFormula, MirrError> {
    if let Some((lhs, rhs)) = inner.split_once(" -> ") {
        let antecedent = parse_expression(lhs.trim()).map_err(|e| {
            MirrError::parse_error(format!("[E155] Property '{name}' antecedent error: {e}"))
        })?;
        let consequent = parse_expression(rhs.trim()).map_err(|e| {
            MirrError::parse_error(format!("[E156] Property '{name}' consequent error: {e}"))
        })?;
        return Ok(PropertyFormula::AlwaysImplies { antecedent, consequent });
    }

    let expr = parse_expression(inner).map_err(|e| {
        MirrError::parse_error(format!("[E157] Property '{name}' formula error: {e}"))
    })?;
...
fn try_parse_followed_by(inner: &str, name: &str) -> Result<Option<PropertyFormula>, MirrError> {
    ...
    let trigger = parse_expression(trigger_str).map_err(|e| {
        MirrError::parse_error(format!("[E165] Property '{name}' trigger error: {e}"))
    })?;
    let response = parse_expression(response_str).map_err(|e| {
        MirrError::parse_error(format!("[E166] Property '{name}' response error: {e}"))
    })?;
