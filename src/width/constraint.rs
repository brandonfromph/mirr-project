...
where
    K: Eq + Hash + Borrow<str>,
{
    use crate::ecs::components::{WidthConstraintComponent, EntityId, PendingSignalRef};
    let mut diagnostics = Vec::new();
    ...
    for i in 0..registry.names.len() {
        ...
        let constraint = if let Some(lit) = &registry.literals[i] {
            let val = match lit.0 {
                crate::ast::types::LiteralValue::Bool(b) => if b { 1 } else { 0 },
            ...
            if let Some(&width) = signals.get(name.as_str()) {
            ...
            match bin.op {
                BinaryOp::Add => WidthConstraintComponent::MaxPlusOne { left: bin.left, right: bin.right },
                BinaryOp::Mul => WidthConstraintComponent::SumOf { left: bin.left, right: bin.right },
                BinaryOp::Sub | BinaryOp::And | BinaryOp::Or | BinaryOp::BitwiseOr | BinaryOp::Xor => {
                ...
                BinaryOp::BitwiseAnd => {
                    if let Some(l_lit) = registry.literals[bin.left.0 as usize].as_ref() {
                        let l_val = match l_lit.0 { crate::ast::types::LiteralValue::Bool(b) => if b { 1 } else { 0 }, crate::ast::types::LiteralValue::Integer(v) => v };
                        WidthConstraintComponent::Narrowed { source: bin.right, narrow_width: min_bits_for(l_val) }
                    } else if let Some(r_lit) = registry.literals[bin.right.0 as usize].as_ref() {
                        let r_val = match r_lit.0 { crate::ast::types::LiteralValue::Bool(b) => if b { 1 } else { 0 }, crate::ast::types::LiteralValue::Integer(v) => v };
                        WidthConstraintComponent::Narrowed { source: bin.left, narrow_width: min_bits_for(r_val) }
                    } else {
                        WidthConstraintComponent::MaxOf { left: bin.left, right: bin.right }
                    }
                }
                BinaryOp::Shl => {
                    if let Some(r_lit) = registry.literals[bin.right.0 as usize].as_ref() {
                        let r_val = match r_lit.0 { crate::ast::types::LiteralValue::Bool(b) => if b { 1 } else { 0 }, crate::ast::types::LiteralValue::Integer(v) => v };
                        WidthConstraintComponent::LeftPlusConst { left: bin.left, shift_amount: r_val as u32 }
                    } else {
                ...
                BinaryOp::Shr => {
                    if let Some(r_lit) = registry.literals[bin.right.0 as usize].as_ref() {
                        let r_val = match r_lit.0 { crate::ast::types::LiteralValue::Bool(b) => if b { 1 } else { 0 }, crate::ast::types::LiteralValue::Integer(v) => v };
                        WidthConstraintComponent::LeftMinusConst { left: bin.left, shift_amount: r_val as u32 }
                    } else {
                        WidthConstraintComponent::SameAs { source: bin.left }
                    }
                }
                BinaryOp::Eq | BinaryOp::Ne | BinaryOp::Lt | BinaryOp::Le | BinaryOp::Gt | BinaryOp::Ge => {
                    WidthConstraintComponent::Boolean
                }
            }
            ...
            WidthConstraintComponent::SumAll { elements: str_lit.fields.iter().map(|f| f.1).collect() }
        } else {
