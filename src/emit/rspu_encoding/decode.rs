...
/// Decode a machine word back into an R-SPU instruction using a target spec.
pub fn decode(word: u64, target: &TargetSpec) -> Result<RspuInstruction, MirrError> {
    let opcode = extract_opcode(word);

    match opcode {
        OP_LOAD_INPUT => {
            let (dst, _src, imm) = extract_i_fields(word, target);
            Ok(RspuInstruction::LoadInput { dst, port: imm as u16 })
        }
        OP_STORE_OUTPUT => {
            let (_dst, src, imm) = extract_i_fields(word, target);
            Ok(RspuInstruction::StoreOutput { src, port: imm as u16 })
        }
        OP_MOV => {
            let (dst, src, _, _) = extract_r_fields(word, target);
            Ok(RspuInstruction::Mov { dst, src })
        }
        OP_LOAD_IMM => {
            let (dst, w, imm) = extract_i_fields(word, target);
            Ok(RspuInstruction::LoadImm { dst, value: imm as u64, width: w as u32 })
        }
        OP_ALU => {
            let (dst, a, b, op_funct) = extract_r_fields(word, target);
            let op = funct_to_alu_op(op_funct)?;
            Ok(RspuInstruction::Alu { op, dst, a, b })
        }
        OP_ALU_IMM => {
            let (dst, a, packed) = extract_i_fields(word, target);
        ...
        OP_ALU_UNARY => {
            let (dst, src, _, funct) = extract_r_fields(word, target);
        ...
        OP_SR_QUERY => {
            let (guard, dst, _, _) = extract_g_fields(word, target);
            Ok(RspuInstruction::SrQuery { dst, guard })
        }
        OP_CTR_INIT => {
            let (guard, cond, imm) = extract_i_fields(word, target);
            Ok(RspuInstruction::CtrInit { guard: guard as u8, target: imm as u64, cond })
        }
        OP_CTR_TICK => {
            let (guard, _, _, _) = extract_g_fields(word, target);
        ...
        OP_GUARD_AND => {
            let (dst, a, b, _) = extract_r_fields(word, target);
        ...
        OP_REFLEX_IF => {
            let (guard, dst, _, src) = extract_g_fields(word, target);
            Ok(RspuInstruction::ReflexIf { guard, dst, src: src as u16 })
        }
        OP_PREV => {
            let (dst, signal, imm) = extract_i_fields(word, target);
            Ok(RspuInstruction::Prev { dst, signal, delay: imm })
        }
        ...
        OP_TAG_LOAD => {
            let (dst, tag, _) = extract_i_fields(word, target);
            Ok(RspuInstruction::TagLoad { dst, tag: tag as u8 })
        }
        OP_TAG_CHECK => {
            let (src, expected, _) = extract_i_fields(word, target);
        ...
        OP_CERTIFY => {
            let (dst, _, _, _) = extract_r_fields(word, target);
        ...
        OP_MATCH => {
            let (dst, src, imm) = extract_i_fields(word, target);
            Ok(RspuInstruction::Match { dst, src, table_offset: imm as u16 })
        }
        OP_INTERVAL_LO => {
            let (dst, src1, _, _) = extract_r_fields(word, target);
        ...
        OP_INTERVAL_CHECK => {
            let (_dst, src1, src2, _) = extract_r_fields(word, target);
