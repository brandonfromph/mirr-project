...
impl RspuProgram {
    ...
    pub fn emit_asm(&self) -> String {
        ...
        if let Some(ref target) = self.target {
            writeln!(out, "; Target:          {}", target.name).unwrap();
        }
        writeln!(out, "; Registers used: {}", self.registers_used).unwrap();
        writeln!(out, "; Guards used:    {}", self.guards_used).unwrap();
        writeln!(out, "; Instructions:   {}", self.instructions.len()).unwrap();
        out.push_str(";\n; Register map:\n");

        for (name, reg) in &self.register_map {
            writeln!(out, ";   R{reg:<3} = {name}").unwrap();
        }
        out.push_str(";\n; Guard map:\n");
        for (name, gid) in &self.guard_map {
            writeln!(out, ";   G{gid:<3} = {name}").unwrap();
        }
        out.push('\n');

        for (i, instr) in self.instructions.iter().enumerate() {
            writeln!(out, "{:>4}:  {}", i, format_instruction(instr)).unwrap();
        }
