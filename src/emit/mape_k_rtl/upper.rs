...
pub(super) fn emit_plan_block(config: &SimConfig, main_clock: &str) -> String {
    ...
    writeln!(sv, "  parameter N_ACTIONS    = {n_act}").unwrap();
    sv.push_str(") (\n");
    ...
    writeln!(sv, "  logic [{}:0] best_idx;", act_w.saturating_sub(1)).unwrap();
    sv.push_str("  logic        found;\n\n");

    sv.push_str("  always_comb begin\n");
    sv.push_str("    best_priority = 8'd0;\n");
    writeln!(sv, "    best_idx      = {}'d0;", act_w.max(1)).unwrap();
    ...
    writeln!(sv, "      selected_action_idx <= {}'d0;", act_w.max(1)).unwrap();
...
pub(super) fn emit_execute_block(config: &SimConfig, main_clock: &str) -> String {
    ...
    writeln!(sv, "  input  logic {},", main_clock).unwrap();
    sv.push_str("  input  logic rst_n,\n");
    writeln!(sv, "  input  logic [{}:0] selected_action_idx,", act_w.saturating_sub(1)).unwrap();
    ...
    for (ai, entry) in config.action_table.iter().enumerate().take(MAX_RTL_ACTIONS) {
        if matches!(entry.action, AdaptationAction::EmergencyStop) {
            writeln!(sv, "        {}'d{ai}: emergency_active <= 1'b1;", act_w.max(1)).unwrap();
        }
    }
    ...
    for i in 0..n_sig.min(MAX_RTL_SIGNALS) {
        writeln!(sv, "      signal_override[{i}] <= 32'd0;").unwrap();
        writeln!(sv, "      override_en[{i}]     <= 1'b0;").unwrap();
    }
    ...
    for (ai, entry) in config.action_table.iter().enumerate().take(MAX_RTL_ACTIONS) {
        writeln!(sv, "        {}'d{ai}: begin", act_w.max(1)).unwrap();
        match &entry.action {
            AdaptationAction::SetSignal { name, value } => {
                let si = sig_idx(name);
                writeln!(sv, "          signal_override[{si}] <= 32'd{value};").unwrap();
                writeln!(sv, "          override_en[{si}]     <= 1'b1;").unwrap();
            }
            ...
            AdaptationAction::EmergencyStop => {
                for i in 0..n_sig.min(MAX_RTL_SIGNALS) {
                    writeln!(sv, "          signal_override[{i}] <= 32'd0;").unwrap();
                    writeln!(sv, "          override_en[{i}]     <= 1'b1;").unwrap();
                }
            }
...
pub(super) fn emit_knowledge_block(config: &SimConfig, main_clock: &str) -> String {
    ...
    writeln!(sv, "  parameter DEPTH = {depth}").unwrap();
    sv.push_str(") (\n");
    writeln!(sv, "  input  logic {},", main_clock).unwrap();
    sv.push_str("  input  logic rst_n,\n");
    sv.push_str("  input  logic        wr_en,\n");
    writeln!(sv, "  input  logic [{}:0] wr_action_idx,", act_w.saturating_sub(1)).unwrap();
    sv.push_str("  input  logic [31:0] wr_tick,\n");
    writeln!(sv, "  output logic [{}:0] count,", addr_w).unwrap();
    sv.push_str("  output logic        full\n");
    sv.push_str(");\n\n");

    let record_w: usize = 32 + act_w.max(1);
    writeln!(sv, "  logic [{}:0] fifo [0:DEPTH-1];", record_w.saturating_sub(1)).unwrap();
    write!(sv, "  logic [{}:0] wr_ptr;\n\n", addr_w.saturating_sub(1)).unwrap();

    sv.push_str("  assign full = (count == DEPTH);\n\n");

    writeln!(sv, "  always_ff @(posedge {} or negedge rst_n) begin", main_clock).unwrap();
    sv.push_str("    if (!rst_n) begin\n");
    writeln!(sv, "      wr_ptr <= {}'d0;", addr_w).unwrap();
...
pub(super) fn emit_mape_k_top(config: &SimConfig, main_clock: &str) -> String {
    ...
    writeln!(sv, "  parameter N_PROPERTIES = {n_prop},").unwrap();
    writeln!(sv, "  parameter N_ACTIONS    = {n_act},").unwrap();
    writeln!(sv, "  parameter K_DEPTH      = {depth}").unwrap();
    sv.push_str(") (\n");
    writeln!(sv, "  input  logic {},", main_clock).unwrap();
    sv.push_str("  input  logic rst_n,\n");
    ...
    writeln!(sv, "  logic [{}:0] violation_vec;", n_prop.max(1).saturating_sub(1)).unwrap();
    write!(sv, "  logic [{}:0] top_violation_idx;\n\n", prop_w.saturating_sub(1)).unwrap();

    sv.push_str("  // Plan -> Execute\n");
    writeln!(sv, "  logic [{}:0] selected_action_idx;", act_w.saturating_sub(1)).unwrap();
    sv.push_str("  logic        action_valid;\n\n");

    sv.push_str("  // Knowledge write channel\n");
    writeln!(sv, "  logic [{}:0] k_count;", addr_w).unwrap();
    ...
    writeln!(sv, "    .TRACE_DEPTH ({trace_depth})").unwrap();
    sv.push_str("  ) u_monitor (\n");
    writeln!(sv, "    .{}          ({}),", main_clock, main_clock).unwrap();
