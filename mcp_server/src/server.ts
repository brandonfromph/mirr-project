...
const handlers: Record<string, (req: express.Request, res: express.Response) => Promise<any>> = {};
// override app.post/get since those are the only verbs we use in this server
const originalPost = app.post.bind(app);
(app as any).post = (path: string, handler: express.RequestHandler) => {
  const method = path.replace(/^\//, "");
  handlers[method] = handler;
  return originalPost(path, handler);
};
const originalGet = app.get.bind(app);
(app as any).get = (path: string, handler: express.RequestHandler) => {
  const method = path.replace(/^\//, "");
  handlers[method] = handler;
...
// the express routes (HTTP routes are defined but never bound in this mode).
  ...
  async function handleStdIoMessage(msg: { id?: number; method?: string; params?: any }) {
    ...
    const res: express.Response = {
      ...
      json(obj: any) { send(msg.id, this.statusCode, obj); },
      send(obj: any) { send(msg.id, this.statusCode, obj); },
      setHeader() {},
    };

    try {
      await handlers[methodName](req, res);
    } catch (err) {
...
app.post("/generate_api_key", (req, res) => {
  ...
  if (!rr.ok) {
    ...
    return res.status(code).json({ error: rr.reason });
  }
  const { id, role } = req.body;
  if (!id || !role) return res.status(400).json({ error: "missing_id_or_role" });
  const crypto = require("crypto");
...
app.get("/list_api_keys", (req, res) => {
  ...
  const list = (CONFIG.api_keys || []).map(k => ({ id: k.id, role: k.role }));
  res.json({ keys: list });
});
...
app.post("/revoke_api_key", (req, res) => {
  ...
  if (!token && !id) return res.status(400).json({ error: "missing_token_or_id" });
  CONFIG.api_keys = (CONFIG.api_keys || []).filter(k => {
...
app.post("/search_files", async (req, res) => {
  try {
    ...
    const opts: any = { cwd: abs, nodir: true };
    if (Array.isArray(ignore)) opts.ignore = ignore;
    const matches = await globAsync(pattern, opts);
    return res.json({ base: abs, matches });
  } catch (err: any) {
    return res.status(500).json({ error: String(err) });
  }
});

// Tool: directory_tree – simple recursive listing
app.post("/directory_tree", async (req, res) => {
  try {
    const { path: relPath, maxDepth = 5 } = req.body;
    if (!isPathAllowed(relPath || ".")) return res.status(403).json({ error: "path_not_allowed" });
    ...
    const walk = async (dir: string, depth: number): Promise<any> => {
      ...
      for (const it of items) {
        if (it.isDirectory()) {
          entries[it.name] = await walk(path.join(dir, it.name), depth - 1);
        } else {
          entries[it.name] = null;
        }
      }
      return entries;
    };
  ...
  try {
    ...
    if (commit) {
      try {
        ...
        return res.json({ path: abs, wrote: true, gitError: e.stderr?.toString?.() ?? e.message });
      }
    }

    return res.json({ path: abs, wrote: true });
  } catch (err: any) {
    return res.status(400).json({ error: String(err.message || err) });
  }
});
