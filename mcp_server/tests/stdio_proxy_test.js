...
async function runTests() {
  ...
  const cfgPath = path.join(__dirname, '..', 'mcp_server', 'config.json');
  try {
  ...
  const write = await sendRequest(proc, { method: 'write_file', params: { path: 'temp_test.txt', content: 'xyz' } });
  assert.strictEqual(write.result.ok, true);
  const read = await sendRequest(proc, { method: 'read_text_file', params: { path: 'temp_test.txt' } });
