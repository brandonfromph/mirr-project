...
use mirrc::ecs::components::{
    AssignmentComponent, BinaryComponent, CyclesComponent, LiteralComponent,
    PrevComponent, ReflexComponent, SignalRefComponent, TypeComponent,
    EntityKind, KindComponent,
};
...
fn solve_nonexpansive_updates_widths() {
    ...
    reg.types[e1.0 as usize] = Some(TypeComponent(mirrc::ast::types::ExtendedType::from_core(SignalType::Unsigned(8))));
    reg.types[e2.0 as usize] = Some(TypeComponent(mirrc::ast::types::ExtendedType::from_core(SignalType::Unsigned(16))));
...
fn solve_expansive_infers_bound_from_guards() {
    ...
    reg.binary_ops[bin_id.0 as usize] = Some(BinaryComponent {
        op: BinaryOp::Add,
        left: prev_id,
        right: lit_id,
    });
    ...
    reg.assignment_comps[a_id.0 as usize] = Some(AssignmentComponent {
        target: sig_id,
        target_index: None,
        value: bin_id,
    });
    
    reg.reflex_comps[reflex_id.0 as usize] = Some(ReflexComponent {
        assignments: vec![a_id],
        guards: vec![guard_id],
        origin: None,
    });
