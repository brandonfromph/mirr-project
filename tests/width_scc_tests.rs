...
use nasa_rust_project::ast::program::{
    Assignment, Guard, MirrProgram, Module, Reflex, SignalDecl,
};
use nasa_rust_project::ast::types::{
    BinaryOp, LiteralValue, SignalKind, SignalType,
};
...
fn acyclic_program_has_no_self_loops() {
    let prog = program("acyclic", vec![
        sig("a", SignalKind::Input, SignalType::Unsigned(8)),
        sig("b", SignalKind::Output, SignalType::Unsigned(8)),
    ], vec![
        guard("g1", signal_expr("a"), 1),
    ], vec![
        reflex("r1", &["g1"], vec![assign("b", signal_expr("a"))]),
    ]);
...
fn self_referencing_prev_creates_self_loop() {
    let prog = program("counter_mod", vec![
        sig("counter", SignalKind::Internal, SignalType::Unsigned(8)),
    ], vec![
        guard("g1", Expr::Literal(LiteralValue::Bool(true)), 1),
    ], vec![
        reflex("r1", &["g1"], vec![
...
fn single_self_loop_detected_as_scc() {
    let prog = program("selfloop", vec![
        sig("x", SignalKind::Internal, SignalType::Unsigned(16)),
    ], vec![
        guard("g1", Expr::Literal(LiteralValue::Bool(true)), 10),
    ], vec![
        reflex("r1", &["g1"], vec![
            assign("x", add(prev("x", 1), int_lit(1))),
        ]),
    ]);
    let result = width::infer_program_widths_with_scc(&prog);
...
fn prev_plus_constant_classified_expansive() {
    let prog = program("expansive", vec![
        sig("counter", SignalKind::Internal, SignalType::Unsigned(8)),
    ], vec![
        guard("g1", Expr::Literal(LiteralValue::Bool(true)), 100),
    ], vec![
        reflex("r1", &["g1"], vec![
            assign("counter", add(prev("counter", 1), int_lit(1))),
        ]),
...
fn pure_prev_chain_classified_nonexpansive() {
    let prog = program("nonexpansive", vec![
        sig("a", SignalKind::Internal, SignalType::Unsigned(8)),
        sig("b", SignalKind::Internal, SignalType::Unsigned(8)),
    ], vec![
        guard("g1", Expr::Literal(LiteralValue::Bool(true)), 1),
...
fn and_in_cycle_classified_nonexpansive() {
    ...
    let prog = program("mask_cycle", vec![
        ...
        reflex("r1", &["g1"], vec![
            assign("x", and_expr(prev("x", 1), signal_expr("mask"))),
        ]),
    ]);
    let result = width::infer_program_widths_with_scc(&prog);
    assert_eq!(result.sccs.len(), 1);
...
fn counter_with_explicit_annotation_resolves() {
    let prog = program("counter_u8", vec![
        sig("counter", SignalKind::Internal, SignalType::Unsigned(8)),
    ], vec![
        guard("g1", Expr::Literal(LiteralValue::Bool(true)), 255),
    ], vec![
        reflex("r1", &["g1"], vec![
            assign("counter", add(prev("counter", 1), int_lit(1))),
        ]),
...
fn counter_with_guard_bound_infers_width() {
    ...
    let prog = program("bounded_counter", vec![
        sig("counter", SignalKind::Internal, SignalType::Unsigned(8)),
    ], vec![
        guard("g1", Expr::Literal(LiteralValue::Bool(true)), 255),
    ], vec![
...
fn counter_with_large_guard_bound_infers_u10() {
    ...
    let prog = program("big_counter", vec![
        sig("counter", SignalKind::Internal, SignalType::Unsigned(10)),
    ], vec![
        guard("g1", Expr::Literal(LiteralValue::Bool(true)), 1000),
    ], vec![
        reflex("r1", &["g1"], vec![
            assign("counter", add(prev("counter", 1), int_lit(1))),
        ]),
...
fn accumulator_plus_sensor_with_annotation() {
    ...
    let prog = program("accumulator", vec![
        ...
        reflex("r1", &["g1"], vec![
            assign("accum", add(prev("accum", 1), signal_expr("sensor"))),
        ]),
    ]);
    let result = width::infer_program_widths_with_scc(&prog);
    assert!(!result.has_errors());
...
fn expansive_scc_error_pinned_by_signal_name() {
    ...
    let prog = program("nobound", vec![
        sig("x", SignalKind::Internal, SignalType::Unsigned(0)),
    ], vec![
        guard("g1", Expr::Literal(LiteralValue::Bool(true)), 0),
    ], vec![
        reflex("r1", &["g1"], vec![
            assign("x", add(prev("x", 1), int_lit(1))),
        ]),
    ...
    let result = width::infer_program_widths_with_scc(&prog);
    assert!(result.has_errors());
    let error_msgs: Vec<&str> = result.scc_diagnostics.iter()
...
fn nonexpansive_no_anchor_produces_error() {
    ...
    assert!(result.has_errors());
    assert!(
        result.scc_diagnostics.iter().any(|d|
            d.message.contains("no width anchor")
        ),
...
fn nonexpansive_solution_verified_minimal() {
    let prog = program("ring_verify", vec![
        sig("a", SignalKind::Internal, SignalType::Unsigned(8)),
        sig("b", SignalKind::Internal, SignalType::Unsigned(8)),
    ], vec![
        guard("g1", Expr::Literal(LiteralValue::Bool(true)), 1),
...
fn program_with_mixed_acyclic_and_cyclic() {
    ...
    let prog = program("mixed_mode", vec![
        sig("a", SignalKind::Input, SignalType::Unsigned(8)),
        sig("b", SignalKind::Output, SignalType::Unsigned(8)),
...
fn phase3_then_phase4b_integration() {
    ...
    let prog = program("simplify_then_scc", vec![
        sig("x", SignalKind::Internal, SignalType::Unsigned(8)),
    ], vec![
        guard("g1", Expr::Literal(LiteralValue::Bool(true)), 255),
    ], vec![
        reflex("r1", &["g1"], vec![
            assign("x", add(prev("x", 1), add(int_lit(1), int_lit(0)))),
        ]),
    ]);
...
fn scc_report_format_includes_signal_names() {
    let prog = program("report_test", vec![
        sig("a", SignalKind::Internal, SignalType::Unsigned(8)),
        sig("b", SignalKind::Internal, SignalType::Unsigned(8)),
    ], vec![
        guard("g1", Expr::Literal(LiteralValue::Bool(true)), 1),
    ...
    let result = width::infer_program_widths_with_scc(&prog);
    let signal_names: Vec<String> = prog.module.signals.iter()
        .map(|s| s.name.clone())
        .collect();
...
fn stats_include_scc_fields() {
    let prog = program("stats_test", vec![
        sig("x", SignalKind::Internal, SignalType::Unsigned(8)),
    ], vec![
        guard("g1", Expr::Literal(LiteralValue::Bool(true)), 10),
    ], vec![
