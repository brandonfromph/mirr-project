...
fn test_noc_router_priority_scheduling() {
    ...
    for i in 0..16 {
        ...
        sim.set_input(18 + i, 0, TypeTag::Unsigned { width: 64 }); // port_tx_data[i] = 0
    }
    sim.set_input(34, 0, TypeTag::Bool); // stream_tx_valid
    sim.set_input(35, 0, TypeTag::Unsigned { width: 64 }); // stream_tx_data
    ...
    sim.set_input(18 + 0, tx_data_0, TypeTag::Unsigned { width: 64 }); // tx_data_0
    ...
    for i in 0..16 {
        ...
        let data = sim.read_output(16 + i).map(|w| w.value).unwrap_or(0);
        if valid || data != 0 {
    ...
    // Check outputs
    let rx_valid_5 = sim.read_output(5).map(|w| w.value != 0).unwrap_or(false);
    let rx_data_5 = sim.read_output(16 + 5).map(|w| w.value).unwrap_or(0);
    ...
    for i in 0..16 {
        ...
        sim.set_input(18 + i, 0, TypeTag::Unsigned { width: 64 });
    }
    sim.set_input(34, 0, TypeTag::Bool); // stream_tx_valid
    sim.set_input(35, 0, TypeTag::Unsigned { width: 64 }); // stream_tx_data

    sim.set_input(2 + 1, 1, TypeTag::Bool); // tx_valid_1 = true
    sim.set_input(18 + 1, tx_data_1, TypeTag::Unsigned { width: 64 }); // tx_data_1
    sim.set_input(2 + 2, 1, TypeTag::Bool); // tx_valid_2 = true
    sim.set_input(18 + 2, tx_data_2, TypeTag::Unsigned { width: 64 }); // tx_data_2

    sim.run_cycle(&prog).expect("Simulation failed on cycle 3");
    sim.run_cycle(&prog).expect("Simulation failed on cycle 4");

    let rx_valid_7 = sim.read_output(7).map(|w| w.value != 0).unwrap_or(false);
    let rx_data_7 = sim.read_output(16 + 7).map(|w| w.value).unwrap_or(0);
    ...
    sim.set_input(2 + 3, 1, TypeTag::Bool); // tx_valid_3 = true
    sim.set_input(18 + 3, tx_data_3, TypeTag::Unsigned { width: 64 }); // tx_data_3
    sim.set_input(2 + 14, 1, TypeTag::Bool); // tx_valid_14 = true
    sim.set_input(18 + 14, tx_data_14, TypeTag::Unsigned { width: 64 }); // tx_data_14

    sim.run_cycle(&prog).expect("Simulation failed on cycle 5");
    sim.run_cycle(&prog).expect("Simulation failed on cycle 6");

    let rx_valid_8 = sim.read_output(8).map(|w| w.value != 0).unwrap_or(false);
    let rx_data_8 = sim.read_output(16 + 8).map(|w| w.value).unwrap_or(0);
